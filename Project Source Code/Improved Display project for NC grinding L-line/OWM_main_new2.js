    ...
    for (var i = 1; i < data.length - 1; i++) {
        ...
        if (!state) {
            ...
            if (comp_flg) {
                ...
                detailData[machine][task].push([logTime, end_date]);
            }
            comp_flg = false;
        }
...
function toSeconds(dateAndTime) {
    const date = new Date(dateAndTime.replace(/-/g, '/'));
    return date.getTime() / 1000;
}

function draw_bargraph(data, workSeconds) {
    const state_date = {};
    const machine_list = Object.keys(data);
    const datasets = [];

    // Initialize state_date with keys
    machine_list.forEach(key => {
        Object.keys(data[key]).forEach(subkey => {
            if (subkey !== '量産' && !state_date[subkey]) {
                state_date[subkey] = [];
            }
        });
    });
    ...
    machine_list.forEach(machine_key => {
        Object.keys(data[machine_key]).forEach(state_key => {
            if (state_key !== '量産') {
                const times = data[machine_key][state_key];
                let durations = 0;
                times.forEach(time => {
                    const start = toSeconds(time[0]);
                    const end = toSeconds(time[1]);
                    durations += ((end - start) / workSeconds) * 100;
                });
                state_date[state_key].push(parseFloat(durations.toFixed(2)));
            }
        });
    });

    // Initialize '待機' state with 100
    machine_list.forEach(() => {
        state_date['待機'] = (state_date['待機'] || []).concat(100);
    });

    // Calculate '待機' state by subtracting other states
    machine_list.forEach((_, index) => {
        Object.keys(state_date).forEach(state_key => {
            if (state_key !== '運転準備' && state_key !== '待機') {
                const newValue = state_date['待機'][index] - (state_date[state_key][index] || 0);
                state_date['待機'][index] = parseFloat(newValue.toFixed(2));
            }
        });
    });

    // Create datasets for the chart
    Object.keys(state_date).forEach(key => {
        if (key !== '運転準備') {
            const color = colors[key] || '#dddddd';
            datasets.push({
                label: key,
                data: state_date[key],
...
function calculateWorkSeconds(startDate, endDate) {
    ...
    for (let i = 0; i < breakTimes.length; i++) {
        let breakStartTime = new Date(startDate.split('T')[0] + 'T' + breakTimes[i].start + ':00');
        let breakEndTime = new Date(startDate.split('T')[0] + 'T' + breakTimes[i].end + ':00');
